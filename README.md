...
4. **✍️ 실시간 인사이트 메모장 (Split Workspace & Autosave)**
   - 전사되는 창과 나란히 필기할 수 있는 2단 분할 레이아웃(Split Pane) 제공.
   - 수업이나 회의 도중 떠오르는 아이디어, 질문, 타임스탬프(`⏱️`)를 자유롭게 메모.
   - 타이핑 즉시 맥북 로컬 및 **Google Drive**로 실시간 자동 동기화.

5. **⚡ Gemini 3.8 Flash 원클릭 종합 강의/회의 노트 생성**
   - 버튼 클릭 한 번으로 **전사문 본문**과 **사용자의 인사이트 메모**를 입체적으로 교차 분석.
   - 핵심 테제, 대주제·소주제별 심층 정리, 사용자 메모와의 융합 해석, 핵심 용어 사전, 심화 질문까지 아우르는 '완성형 종합 학술 노트' 자동 직조.

6. **💡 AI 핵심 요약 & 액션 아이템 추출**
   - 3~5개 글머리 기호 형태의 핵심 논점 요약.
   - 체크리스트 형태의 후속 액션 아이템 자동 도출.

7. **📱 아이폰 연속성 마이크(Continuity Mic) 무선 연동**
   - Mac 근처에 있는 iPhone을 고음질 외장 마이크로 자동 감지 (`📱 아이폰 마이크 감지됨` 배지).
   - 아이폰을 강의대나 발표자 근처에 두고, 맥북으로 실시간 전사 가능.

8. **📅 구글 캘린더 연동 (수업/회의 제목 자동 입력)**
   - 구글 캘린더 iCal 비공개 주소를 등록해 두면, 녹음 시작 시 **현재 진행 중이거나 곧 시작하는 일정의 제목을 노트 제목으로 자동 입력**.

9. **☁️ 구글 드라이브(Google Drive) 자동 클라우드 백업**
   - 녹음이 끝나는 즉시 원본 오디오(`.wav`)와 완성된 마크다운 회의록(`.md`, 인사이트 메모 및 종합 정리 노트 포함)이 Mac의 구글 드라이브(`내 드라이브/Gemini_Transcribe_녹음`)로 자동 저장되어 아이폰·아이패드에서도 즉시 열람 가능.

10. **⏸️ 녹음 일시정지 & 재개**
   - 회의 중간 휴식 시 타이머와 오디오 전송을 잠시 멈추고 이어서 녹음 가능.

11. **🖥️ Mac 독립 데스크톱 앱 (`Gemini Transcribe.app`)**
